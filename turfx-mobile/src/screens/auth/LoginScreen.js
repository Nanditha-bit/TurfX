...
import { Eye, EyeOff, User, Trophy, Globe } from 'lucide-react-native';
...
export default function LoginScreen({ navigation }) {
  ...
  const reset = () => {
    setPhone(''); setPassword(''); setErr('');
    setRegName(''); setRegPhone(''); setRegEmail('');
    setRegPassword(''); setRegBusiness('');
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (mode === 'partner') {
        // If account isn't owner/admin yet, upgrade it to owner
        if (role !== 'owner' && role !== 'admin') {
          try {
            ...
            await login(upgradeRes.data.user, upgradeRes.data.token);
          } catch (upgradeErr) {
            // Fallback — log in anyway, backend will handle role
            await login(res.data.user, res.data.token);
          }
        } else {
          await login(res.data.user, res.data.token);
        }
        navigation.replace('PartnerApp');
      } else {
        if (role === 'owner' || role === 'admin') {
          setErr('This is a partner/admin account. Please use Partner Login.');
          setLoading(false);
          return;
        }
        await login(res.data.user, res.data.token);
  ...
  const handleRegister = async () => {
    if (!regName.trim())        { setErr('Enter your name'); return; }
    if (regPhone.length < 10)   { setErr('Enter a valid 10-digit mobile number'); return; }
    ...
    try {
      ...
      // For partner, call become-partner to guarantee owner role + fresh token
      if (role === 'owner') {
        try {
          const upgradeRes = await axios.post(
            `${API}/auth/become-partner`, {},
          ...
          // become-partner failed — still log in with the register token (role already owner)
          await login(res.data.user, res.data.token);
        }
      } else {
        await login(res.data.user, res.data.token);
      ...
      const errorMsg = e.response?.data?.msg
        || (!e.response ? 'Cannot connect to server.' : 'Registration failed. Please try again.');
      setErr(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.modeSwitcher}>
          <TouchableOpacity
            ...
            onPress={() => { setMode('user'); setTab('login'); reset(); }}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => { setMode('partner'); setTab('login'); reset(); }}
          >
            <View style={styles.modeBtnContent}>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        ...
        {/* ── LOGIN FORM (user + partner) ── */}
        {tab === 'login' && (
          <>
            <Text style={styles.formTitle}>{mode === 'partner' ? 'Partner Login' : 'Welcome Back'}</Text>
            {mode === 'partner' && (
              <Text style={styles.formSub}>Sign in with your venue owner account</Text>
            )}
            ...
            <View style={styles.phoneRow}>
              ...
              <TextInput
                style={styles.phoneInput} placeholder="Mobile Number" value={phone}
            ...
            <View style={styles.passwordRow}>
              <TextInput
                style={styles.passwordInput} placeholder="Password" value={password}
            ...
            <TouchableOpacity style={styles.btn} onPress={handleLogin} disabled={loading}>
              {loading
                ...
                : <Text style={styles.btnText}>{mode === 'partner' ? 'Login as Partner' : 'Login'}</Text>
              }
            </TouchableOpacity>
          </>
        )}

        {/* ── REGISTER FORM (user + partner) ── */}
        {tab === 'register' && (
          <>
            ...
            {mode === 'partner' && (
              <TextInput style={styles.input} placeholder="Business / Venue Name (optional)"
                value={regBusiness}
                onChangeText={t => { setRegBusiness(t); setErr(''); }}
            ...
            <View style={styles.phoneRow}>
              ...
              <TextInput
                style={styles.phoneInput} placeholder="Mobile Number *" value={regPhone}
            ...
            <TextInput style={styles.input}
              placeholder={mode === 'partner' ? 'Email Address (optional)' : 'Email (optional)'}
              value={regEmail}
            ...
            <View style={styles.passwordRow}>
              <TextInput
                style={styles.passwordInput} placeholder="Password (min 6 chars) *"
                value={regPassword}
            ...
            {mode === 'partner' && (
              <View style={styles.hint}>
                <Text style={styles.hintText}>After registering, add your venues from the Dashboard</Text>
              </View>
            )}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  formTitle: { fontSize: 24, fontWeight: '900', color: COLORS.dark, marginBottom: 4 },
  formSub:   { fontSize: 13, color: COLORS.textMuted, fontWeight: '500', marginBottom: 16 },
